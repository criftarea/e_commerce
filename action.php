<?php
	...
	$harga = $_POST['harga'];
	$stok = $_POST['stok'];
	$update_gambar = ""; //tambahkan variable update
	
	if(!empty($_FILES["file"]["name"])){ //jika file tidak kosong, jika benar maka akan upload
		$nama_file = $_FILES["file"]["name"];
		move_uploaded_file($_FILES["file"]["tmp_name"], "../../image/barang/".$nama_file);
		
		$update_gambar = ", gambar='$nama_file'"; //digunakan untuk proses update
	}
